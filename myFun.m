...
%% feature with depth
x_bar_k = featureCurrent(1:k,1)./featureCurrent(1:k,3); % [k 1]
y_bar_k = featureCurrent(1:k,2)./featureCurrent(1:k,3); % [k 1]
...
y_bar_k = featureCurrent(k+1:size(featureCurrent,1),2); % [M-k 1]
x_bar_k = featureCurrent(k+1:size(featureCurrent,1),1); % [M-k 1]
