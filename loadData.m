...
%test path
fpathRGB = 'rgbd_dataset_freiburg1_xyz\rgb\';
fpathDEP = 'rgbd_dataset_freiburg1_xyz\depth\';
data = cell(1,2);
%read images
...
data{1} = zeros(480,640,RGB_file_num);
data{2} = zeros(480,640,DEP_file_num);
for i = 1 : RGB_file_num
    data{1}(:,:,i) = im2double(rgb2gray(imread(strcat(fpathRGB, RGB_imagefiles(i).name))));
end
for i = 1 : DEP_file_num
    data{2}(:,:,i) = double(imread(strcat(fpathDEP, DEP_imgefiles(i).name)))/5000.0;
end